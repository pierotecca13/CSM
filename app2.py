"""
CSM Dashboard v2 — app2 (versión nueva)
Incluye: JIRA Cloud, MySQL, exportación Excel/PDF/PPTX, DB Query para PPT
Correr: python app2.py
"""
import os, re, math, socket, json, io
...
app = Flask(__name__, static_folder="static2", static_url_path="/static2")
CORS(app)
...
# ── DB helpers: resolución de credenciales por cliente ───────────
def client_to_prefix(name):
    """'Gador - Lab' → 'GADOR_LAB', 'Knop' → 'KNOP'"""
    return re.sub(r'[^A-Za-z0-9]+', '_', name).strip('_').upper()

def get_db_creds(client_name=""):
    """
    Busca credenciales DB por cliente en el .env.
    Intenta en orden:
      1. Prefijo completo:    GADOR_LAB_DB_NAME  ("Gador - Lab")
      2. Primera palabra:     GADOR_DB_NAME
      3. Default:             DB_NAME
    """
    prefixes = []
    if client_name:
        prefixes.append(client_to_prefix(client_name))          # GADOR_LAB
        first_word = re.split(r'[\s\-_,]+', client_name)[0]
        short = client_to_prefix(first_word)
        if short and short != prefixes[0]:
            prefixes.append(short)                               # GADOR

    def env(key, default=""):
        for p in prefixes:
            val = os.getenv(f"{p}_{key}")
            if val:
                return val
...
# ── Resolver nombres parciales de org a nombres exactos en JIRA ──
def resolve_org_names(url, email, token, search_names):
    """
    Pagina el endpoint de organizaciones de JIRA Service Desk y filtra
    localmente por substring. JIRA ignora el searchTerm en algunas instancias,
    por eso hay que recorrer todas las páginas.
    Retorna un set con los nombres exactos encontrados, o None si falla.
    """
    # Primero cargar todas las orgs una sola vez (con paginación)
    all_orgs = []
    start = 0
    limit = 50
    max_orgs = 1000  # tope de seguridad
    try:
        while len(all_orgs) < max_orgs:
            r = requests.get(
                f"{url.rstrip('/')}/rest/servicedeskapi/organization",
            ...
            all_orgs.extend(values)
            if data.get("isLastPage", True) or len(values) < limit:
                break
            start += limit
        print(f"[JIRA] Total orgs cargadas: {len(all_orgs)}")
    except Exception as e:
        print(f"[JIRA] Error cargando orgs: {e}")
        return None

    if not all_orgs:
        return None

    # Filtrar por substring contra cada término buscado
    resolved = set()
    for name in search_names:
        for org in all_orgs:
            org_name = org.get("name", "")
            if org_name and name.lower() in org_name.lower():
                resolved.add(org_name)
                print(f"[JIRA] Org resuelta: '{name}' → '{org_name}'")
...
def process_issues(issues, sla_field, client_filter=None, days=90):
    ...
    for issue in issues:
        ...
        # Si se definió filtro de clientes, incluir SOLO los que coincidan (sin excepciones)
        if client_filter:
        ...
        if created:
            try:
                ...
                is_last30 = (now_utc - created_dt_aware).days <= days
            except Exception:
                pass
...
def get_metrics():
    ...
    # Límite de días según filtro
    if client_filter:
        days = min(days, 180)
    else:
        days = min(days, 30)
    print(f"[JIRA] Días efectivos: {days}")
    ...
    try:
        ...
        # Resolver nombres parciales a nombres exactos de org en JIRA
        exact_org_filter = None
        if client_filter:
            exact_org_filter = resolve_org_names(url, email, token, client_filter)
            if exact_org_filter:
                print(f"[JIRA] Usando JQL con orgs exactas: {exact_org_filter}")
            else:
                print(f"[JIRA] No se resolvieron orgs exactas — se filtrará localmente")

        def build_jql(extra=""):
            base = f"{proj_filter}{extra}"
            if exact_org_filter:
                org_names = ", ".join(f'"{n}"' for n in exact_org_filter)
                # Intentar con organization (JSM) y cf[11302] (Organizaciones)
                return f'({base}) AND (organization in ({org_names}) OR cf[11302] in ({org_names}))'
            return base
...
# ════════════════════════════════════════════════════════
# DB QUERY ENDPOINT (para slide Actividad Operativa en PPT)
# ════════════════════════════════════════════════════════
...
def db_query():
    try:
        ...
        return jsonify({"ok": False, "error": "mysql-connector-python no instalado. Ejecuta: pip install mysql-connector-python"}), 500

    b = request.json or {}
    days_req    = int(b.get("days", 30))
    client_name = b.get("clientName", "")

    # Calcular rango de fechas igual que el período JIRA
    tz_ar = timezone(timedelta(hours=-3))
    now   = datetime.now(tz_ar)
    end_dt   = now
    start_dt = now - timedelta(days=days_req)
    start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
    end_str   = end_dt.strftime("%Y-%m-%d %H:%M:%S")

    # Recargar .env en cada request (no requiere reiniciar el servidor)
    try:
        from dotenv import load_dotenv
    ...
    # Resolver credenciales por cliente (fallback a defaults si no hay vars específicas)
    creds = get_db_creds(client_name)
    host, port, user, pwd, db = creds["host"], creds["port"], creds["user"], creds["pwd"], creds["db"]
    print(f"[DB] Cliente: '{client_name}' → {host}:{port} db={db} user={user}")

    # NOT LIKE movido a HAVING para que el optimizer pueda usar índice en timestamp+tipo
    query = """
        SELECT tipo AS action, COUNT(*) AS qty
        FROM log_estado
        WHERE timestamp >= %s AND timestamp < %s
          AND tipo NOT IN ('Validada','Devuelve ANMAT','Recibe ANMAT','Envio Item EDI',
                           'Stock','Despacho','Despacho Anulado','Rechazada')
        GROUP BY tipo
        HAVING action NOT LIKE 'inf%%'
        ORDER BY qty DESC
    """
    try:
        conn = mysql.connector.connect(
        ...
        cursor.close()
        conn.close()
        print(f"[DB] Query OK — {len(rows)} filas")
        return jsonify({
            "ok": True,
            "data": [{"action": r["action"], "qty": int(r["qty"])} for r in rows],
...
# ════════════════════════════════════════════════════════
# DB QUERY ENDPOINT — Detalle de Unidades Creadas (para slide PPT)
# ════════════════════════════════════════════════════════

@app.route("/api/db-query-unidades", methods=["POST"])
def db_query_unidades():
    try:
        import mysql.connector
    except ImportError:
        return jsonify({"ok": False, "error": "mysql-connector-python no instalado. Ejecuta: pip install mysql-connector-python"}), 500
    ...
    creds = get_db_creds(client_name)
    host, port, user, pwd, db = creds["host"], creds["port"], creds["user"], creds["pwd"], creds["db"]
    print(f"[DB-UNIDADES] Cliente: '{client_name}' → {host}:{port} db={db} user={user}")

    # Estrategia: pre-agregar item POR LOTE antes de joinear.
    # En lugar de expandir todos los registros de item junto a lote+medicamento
    # y luego contar, primero colapsamos item a (cod_lote, mes, cnt) — conjunto
    # mucho más chico — y recién ahí traemos nombre/GTIN desde medicamento.
    query = """
        SELECT i_agg.Mes,
               m.nombre        AS Producto,
               MAX(m.bc_ean_1) AS GTIN,
               SUM(i_agg.cnt)  AS qty
        FROM (
            SELECT cod_lote,
            ...
            WHERE fecha_alta >= %s
              AND fecha_alta <  %s
            GROUP BY cod_lote, DATE_FORMAT(fecha_alta, '%Y-%m')
        ) AS i_agg
        JOIN lote        l ON l.cod_lote      = i_agg.cod_lote
        JOIN medicamento m ON m.id_medicamento = l.id_medicamento
        GROUP BY i_agg.Mes, m.id_medicamento, m.nombre
        ORDER BY i_agg.Mes, m.nombre
    """
    try:
        conn = mysql.connector.connect(
            host=host, port=port, user=user, password=pwd,
        ...
        cursor.close()
        conn.close()
        print(f"[DB-UNIDADES] Query OK — {len(rows)} filas")
        return jsonify({
            "ok": True,
            "data": [{"mes": r["Mes"], "producto": r["Producto"], "gtin": str(r["GTIN"] or ""), "qty": int(r["qty"])} for r in rows],
            "period": {"start": start_str, "end": end_str},
        })
    except Exception as e:
        print(f"[DB-UNIDADES] Error: {e}")
        return jsonify({"ok": False, "error": str(e)}), 500

# ════════════════════════════════════════════════════════
# EXCEL EXPORT — Detalle de Unidades Creadas (GET, para hipervínculo en PPT)
# ════════════════════════════════════════════════════════

@app.route("/api/export/unidades-excel", methods=["GET"])
def export_unidades_excel():
    try:
        import openpyxl
    ...
    days_req    = int(request.args.get("days", 30))
    client_name = request.args.get("clientName", "")

    tz_ar = timezone(timedelta(hours=-3))
    now   = datetime.now(tz_ar)
    start_str = (now - timedelta(days=days_req)).strftime("%Y-%m-%d %H:%M:%S")
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Detalle Unidades Creadas"

    navy_fill = PatternFill("solid", fgColor="0F2044")
    header_font = Font(color="FFFFFF", bold=True, size=11)
    headers = ["Mes", "Producto", "GTIN", "Cantidad"]
    for col, h in enumerate(headers, 1):
        cell = ws.cell(row=1, column=col, value=h)
        cell.fill = navy_fill
        cell.font = header_font
    ...
    for row_i, r in enumerate(rows, 2):
        ws.cell(row=row_i, column=1, value=r["Mes"])
        ws.cell(row=row_i, column=2, value=r["Producto"])
        ws.cell(row=row_i, column=3, value=str(r["GTIN"] or ""))
        ws.cell(row=row_i, column=4, value=int(r["Cantidad"]))

    col_widths = [12, 40, 20, 14]
    ...
    wb.save(out)
    out.seek(0)
    fname = f"{client_name or 'unidades'}_detalle_{datetime.now().strftime('%Y%m%d')}.xlsx"
...
if __name__ == "__main__":
    try:
        ...
        ip = "TU_IP_LOCAL"
    print("=" * 55)
    print("  CSM Dashboard v2 — app2 (nueva versión)")
    print("=" * 55)
